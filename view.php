...
document.addEventListener('DOMContentLoaded', (event) => {
    const metamaskButton = document.getElementById('metamaskButton');
    metamaskButton.addEventListener('click', async function() {
        metamaskButton.textContent = "Loading..."; // Change button label to "Loading..."
        if (typeof window.ethereum !== 'undefined') {
            try {
                const chainId = await ethereum.request({ method: 'eth_chainId' });
                console.log('ID', chainId);
                if (chainId !== '0x66eee') {
                    alert('Please connect to the Arbitrum/Sepolia Mainnet');
                    metamaskButton.textContent = "Certify!"; // Reset button label if wrong network
                    return;
                }
                const accounts = await ethereum.request({ method: 'eth_requestAccounts' });
                const userAddress = accounts[0];
                ...
                await sendDataToServer(signature, userAddress, hash);
                metamaskButton.textContent = "Certify!"; // Reset button label after operation
                
            } catch (error) {
                console.error(error);
                metamaskButton.textContent = "Certify!"; // Reset button label on error
            }
        } else {
            console.log('MetaMask is not installed. Please consider installing it.');
            metamaskButton.textContent = "Certify!"; // Reset button label if MetaMask is not installed
        }
    });
});
